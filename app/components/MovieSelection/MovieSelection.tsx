...
export default function MovieSelection({year, handleMovieSelection} : MovieSelectionProps) {
    ...
    useEffect(() => {
        ...
        const addMorePosters = async () => {
            const response = await fetch(`http://127.0.0.1:5000/movies/fill/${selectedYear}`)
                ...
                const enrichedMovies: Movie[] = await Promise.all(
                        moviesByYear.map( async (movie) => {
                            ...
                            const response = await fetch(`http://127.0.0.1:5000/api/${movie.imdb_id}`, {
                                method: 'POST',
                                headers: {
        ...
        const getMoviesByYear = async () => {
            ...
            try {
                ...
                const response = await fetch(`http://127.0.0.1:5000/movies/${selectedYear}`)
                addMorePosters()
