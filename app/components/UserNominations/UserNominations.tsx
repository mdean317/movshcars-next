...
export default function UserNominations({ year, category, user, lastUpdate, setLastUpdate, setCanUserAdd}: UserNominationsProps) {
    ...
    const handleDeleteClick = async (index: number) => {
        ...
        try {
            ...
            const response = await fetch((`http://127.0.0.1:4000/usernoms/delete/${encodeURIComponent(nomToDelete.user_id)}/${encodeURIComponent(nomToDelete.nomination_id)}`),
                { method: 'DELETE' });
    ...
    useEffect(() => {
        ...
        const getUserNominations = async () => {
            ...
            if (user.username === '') {
                ...
                const response = await fetch(`http://127.0.0.1:4000/usernoms?username=${encodeURIComponent(user.username.trim())}&year=${year}&category=${encodeURIComponent(category.category_id)}`);
